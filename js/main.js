/**
 * MUMWA PHARMACY — Main JS
 * Renders dynamic sections from MUMWA_CONFIG and wires up
...
(function () {
  ...
  function renderBrand() {
    $$('[data-brand-logo]').forEach(el => {
      el.src = cfg.business.logo;
      el.alt = cfg.business.name + ' logo';
    });

    $$('[data-business-name]').forEach(el => {
      el.textContent = cfg.business.name;
    });

    $$('[data-business-desc]').forEach(el => {
      el.textContent = cfg.business.description;
    });

    $$('[data-year]').forEach(el => {
      el.textContent = cfg.business.year;
    });
  }
  ...
  function renderPhones() {
    ...
    if (list) {
      list.innerHTML = cfg.business.phones.map(p =>
    ...
    if (footPhones) {
      footPhones.innerHTML = cfg.business.phones.map(p =>
        `<li>${iconSvg('phone')}<a href="${p.href}">${p.display}</a></li>`
      ).join('');
    }
  }
  ...
  function renderEmail() {
    $$('[data-email-link]').forEach(el => {
      el.href = 'mailto:' + cfg.business.email;
      el.textContent = cfg.business.email;
    });
  }

  function renderAddress() {
    $$('[data-address-line1]').forEach(el => {
      el.textContent = cfg.business.address.line1;
    });

    $$('[data-address-line2]').forEach(el => {
      el.textContent = cfg.business.address.line2;
    });
  }

  function renderWhatsapp() {
    $$('[data-whatsapp-href]').forEach(el => {
      el.href = cfg.business.whatsapp;
    });
  }
    ...
    el.innerHTML = cfg.categories.map((c, i) => `
      <div class="category-card reveal" style="--i:${i}">
        ...
        <div class="cat-body">
          ...
          <div class="cat-foot">
            <a class="btn btn-outline-dark btn-sm" href="#contact">
    ...
    el.innerHTML = cfg.showcase.map((p, i) => `
      <div class="product-card reveal" style="--i:${i}">
        ...
        <div class="prod-body">
          ...
          <a class="btn btn-ghost btn-sm btn-block" href="#contact">
            Contact Us
          </a>
    ...
    el.innerHTML = cfg.branches.map((b, i) => {
      ...
      <div class="branch-card reveal" style="--i:${i}">
        ...
        <div class="branch-body">
          ...
          <a class="branch-phone" href="${b.phone.href}">
            ${b.phone.display}
          </a>
  ...
  function renderMap() {
    ...
    if (dirBtn) {
      dirBtn.href = cfg.business.mapDirectionsUrl &&
        cfg.business.mapDirectionsUrl.trim()
        ? cfg.business.mapDirectionsUrl
        : 'https://www.google.com/maps/search/' +
          encodeURIComponent(
            cfg.business.address.line1 +
            ', ' +
            cfg.business.address.line2
          );
    }
  }
  ...
  function initForm() {
    ...
    const rules = {
      ...
      /*
       * FIXED:
       * The previous regex contained invalid **\\.** syntax.
       */
      email: v =>
        v.trim() === '' ||
        /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.trim()) ||
  ...
  function init() {
    ...
    if ($('#currentYear')) {
      $('#currentYear').textContent = cfg.business.year;
    }
  }
  ...
  /*
   * Remove the loader once the page and its resources have loaded.
   */
  ...
  /*
   * Safety fallback:
   * Never leave the user stuck on the loading screen indefinitely.
   */
